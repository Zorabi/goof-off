...
- **EPUB**：可在「排版」中切换滚动或翻页模式，并调整字号、行距和字体；同时支持目录与全书搜索。
- **PDF**：打开「PDF 适配」，可选择适宽、适页或 25%–400% 自定义缩放；如果文件内置书签目录，底栏还会显示目录按钮。
...
| **主体已隐藏并穿透**   | 先将鼠标移到顶栏或底栏的按钮区域，唤回主体并退出鼠标穿透，再按上述方式拖动。空白拖拽区本身不会唤回隐藏的主体。 |
...
3. 打开顶栏的「更多」菜单，按需开启 **工具栏自动隐藏** 或 **主体自动隐藏**。开启主体自动隐藏时，应用会同时开启工具栏自动隐藏；之后仍可单独关闭工具栏自动隐藏，不影响主体自动隐藏。
...
- **唤回主体**：主体隐藏后，必须将鼠标移到顶栏或底栏的按钮区域，才能恢复阅读内容。仅靠近顶栏下沿或底栏上沿只能唤回工具栏；点击下层窗口或按老板键也无法唤回主体。
- **网页媒体**：网页主体隐藏前，正在播放的音频和视频会自动暂停；唤回主体后不会自动继续播放。

> 自动隐藏仅在已开启「背景隐去」的常规网页或文件阅读界面中生效。主页和历史页可以提前设置，精简模式不显示这些选项。这两个开关只在当前运行期间保留，不会写入长期配置。关闭背景隐去会暂时停用自动隐藏，但不会重置本次运行中的选择；再次开启后，原设置会继续生效。
...
> 表中的「恢复」仅指重新显示由老板键隐藏的整个窗口，不包括被主体自动隐藏的阅读内容；后者需要将鼠标移到顶栏或底栏的按钮区域才能唤回。
...
| **EPUB**       | 目录、全书搜索、滚动 / 翻页双模式、字号 / 行距 / 字体、自动翻页、精简模式；不支持受 DRM 保护的加密 EPUB                               |
| **PDF**        | 文件内书签目录、适宽 / 适页 / 25%–400% 缩放、页码跳转、按需流式加载；不支持全文搜索与精简模式                                        |
...
- **视觉**：自动 / 浅色 / 深色主题、隐身阅读、界面淡化强度，以及 TXT / EPUB 的文字颜色与渐变背景。
...
如果窗口仍在，只是阅读内容或工具栏消失，说明自动隐藏已经生效。请将鼠标移到顶栏或底栏的按钮区域；老板键无法唤回自动隐藏的内容。如果整个窗口都已消失，请再次按下隐藏 / 恢复老板键（默认 `Ctrl+\` 或 `⌘+\`），也可以点击系统托盘中的 Goof Off 图标并选择「恢复显示」。
